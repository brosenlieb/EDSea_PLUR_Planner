...
def generate_schedule(payload: ScheduleRequest, db: Session = Depends(get_db)):
    ...
    # Fetch all performances for combined pool with eager loading for stage and artist
    db_performances = (
        db.query(Performance)
        .options(
            joinedload(Performance.artist),
            joinedload(Performance.stage)
        )
        .filter(Performance.artist_id.in_(target_artist_ids))
        .all()
    )
    ...
    for p in db_performances:
        performances_for_solver.append({
            ...
            "location": p.stage.location_name,  # <-- FIXED: changed from location to location_name
